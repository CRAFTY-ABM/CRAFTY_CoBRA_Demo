...
simp$batchcreation$inputdatadirs$aftparams	<- paste(simp$batchcreation$inputdatadir, "/setC/agents", sep="")
simp$batchcreation$inputdatadirs$production	<- paste("setC/production/", sep="")
...
simp$dirs$param$getparamdir <- function(simp, datatype = NULL) {
	return <- paste(simp$dirs$data,
			if (is.null(datatype)) { 
						...
						paste("setC/worlds", simp$sim$worldname,
						...
						paste("setC/agents", sep="/")
					} else if (datatype %in% c("productivities")) {
						paste("setC/production", sep="/")
