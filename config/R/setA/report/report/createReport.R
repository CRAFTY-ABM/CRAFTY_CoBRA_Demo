...
sh.ensurePath(paste(simp$dirs$simp, "/", preserve$folder, "/report/", preserve$task,"/", sep=""))
preserve$wd <- getwd()
setwd(paste(simp$dirs$simp, "/", preserve$folder, "/report/", preserve$task,"/", sep=""))
simp$sim$id 	<- c(paste(preserve$run, "-", preserve$seed, sep=""))

rnwFile =  paste(simp$dirs$simp, "/", preserve$folder, "/report/report/", "craftyr_report_", 
		"RUN", "-SEED.Rnw", sep="")
...
simp$sim$id 	<- c(paste(preserve$run, "-", preserve$seed, sep=""))
texFile =  paste(simp$dirs$simp, "/", preserve$folder, "/report/", preserve$task,"/", "craftyr_report_", 
		"RUN", "-SEED.tex", sep="")
...
texi2dvi(file= texFile, pdf= TRUE, clean = FALSE, quiet = FALSE)
